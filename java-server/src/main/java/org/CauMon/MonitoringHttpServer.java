    ...
    private class RootHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            String html = "<!doctype html><html lang='ja'><head><meta charset='utf-8'>"
                    ...
                    + ".card{background:var(--bg);border:1px solid var(--border);border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,.04);padding:24px;}"
                    + "h1{font-size:20px;margin:0 0 8px 0;}"
                    + "p.desc{margin:0 0 20px 0;color:var(--muted);}"
                    + "label{display:block;margin:16px 0 6px;font-weight:600;}"
                    + "input{width:100%;padding:10px 12px;border:1px solid var(--border);border-radius:8px;background:#fff;transition:border .2s,box-shadow .2s;}"
                    + "input:focus{outline:none;border-color:var(--ring);box-shadow:0 0 0 3px rgba(147,197,253,.45);}"
                    ...
                    + "<h1>監視サーバー制御</h1>"
                    + "<p class='desc'>白基調のシンプルなUI。必要項目を入力して操作してください。</p>"
                    + "<label for='signals'>シグナル名（カンマ区切り）<span style='color:var(--muted);font-weight:normal;margin-left:8px;'>※ time は含めないでください</span></label>"
                    + "<input id='signals' value='temp,cooling'/>"
                    + "<label for='phi'>STL式（φ）</label>"
                    + "<input id='phi' value=\"alw_[0,10](temp[t] < 100)\"/>"
                    + "<div class='row'>"
